...
export class RateLimit<K = string> {
  ...
  public i = 0;
  /**
   * Whether this entry is overloaded or not.
   */
  public get isOverloaded(): boolean {
    if (!this.manager.overload?.enabled)
      throw new Error("Overload is disabled");
    if (!this.limited)
      throw new Error("Cannot check if a non-limited bucket is overloaded");
    this.i++;
    return this.i == this.manager.overload.limit;
  }
  /**
